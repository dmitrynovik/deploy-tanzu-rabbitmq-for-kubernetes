...
helm install $KUBE_PROMETHEUS_STACK_NAME "kube-prometheus-stack-$KUBE_PROMETHEUS_STACK_VERSION.tgz" \
  --namespace $KUBE_PROMETHEUS_STACK_NAMESPACE \
  --create-namespace \
  --wait \
  --set "defaultRules.create=false" \
...
printf "!!! DO NOT FORGET TO INSTALL https://github.com/rabbitmq/cluster-operator/releases/latest/download/cluster-operator.yml !!!" "$ORANGE"